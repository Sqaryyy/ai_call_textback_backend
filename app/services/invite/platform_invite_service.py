# ============================================================================
# FILE: app/services/invite/platform_invite_service.py
# Platform invite service - for onboarding new business owners
# ============================================================================
...
class PlatformInviteService:
    """Service layer for platform invite operations (admin creates for new owners)."""
    ...
    def create_platform_invite(
        ...
        Args:
            db: Database session
            created_by: Platform admin user ID who created the invite
        ...
        invite = Invite(
            ...
            business_id=None,  # Platform invites have no business
            created_by=created_by,
            role="owner",  # Platform invites always create owners
    ...
    def validate_platform_invite(
        ...
        if not invite:
            return False, "Platform invite not found", None
    ...
    @staticmethod
    def use_platform_invite(
            db: Session,
            invite_id: UUID
    ) -> bool:
        """
        Mark a platform invite as used (increment usage count).
    ...
    def revoke_platform_invite(
        ...
        """
        Deactivate a platform invite (cannot be used anymore).
    ...
    def list_platform_invites(
        ...
        """
        query = db.query(Invite).filter(Invite.invite_type == InviteType.PLATFORM)
    ...
    @staticmethod
    def get_platform_invite_stats(db: Session) -> Dict[str, Any]:
        """Get statistics about platform invites."""
        invites: List[Invite] = cast(
            List[Invite],
            db.query(Invite).filter(Invite.invite_type == InviteType.PLATFORM).all()
        )
    ...
    @staticmethod
    def extend_platform_invite_expiration(
            db: Session,
            invite_id: UUID,
            additional_days: int
    ) -> Optional[Invite]:
        """
        Extend a platform invite's expiration date.
